...
Thinking 模式开关的 token 消耗对比：

| 任务 | Thinking 模式 | 预估 token/次 | 选择理由 |
|---|---|---|---|
...
本系统在多个环节部署了防幻觉措施，确保 LLM 输出的可靠性：
...
这些策略形成多层防护：结构化输出保障格式正确性，枚举约束保障语义合法性，原文校验保障事实一致性，空结果策略保障不确定时不输出错误信息。
